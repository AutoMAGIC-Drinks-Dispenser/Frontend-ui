...
export const LoginPage: React.FC = () => {
  ...
  const [showPostmixModal, setShowPostmixModal] = useState(false);
  const [showPremixModal, setShowPremixModal] = useState(false);
  useEffect(() => {
      ...
      try {
        ...
        if (message.type === 'rfid') {
          ...
          if (rfidData === "refill faxe") {
            setShowPostmixModal(true);
            setShowPremixModal(false);
          } else if (rfidData === "refill mix") {
            setShowPremixModal(true);
            setShowPostmixModal(false);
          } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      <div className="p-8 bg-white rounded-lg shadow-md text-center">
        ...
        <RefillPostmixModal 
          showModal={showPostmixModal} 
          onClose={() => setShowPostmixModal(false)} 
        />
        
        <RefillPremixModal 
          showModal={showPremixModal} 
          onClose={() => setShowPremixModal(false)} 
        />
      </div>
    </div>
  );
